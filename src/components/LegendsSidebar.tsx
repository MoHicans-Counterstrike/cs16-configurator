...
// Compact sidebar — grouped by clan. Hover opens story card to the right.
  ...
  return (
    <div className="mt-4 bg-zinc-900/50 border border-zinc-800 rounded-lg overflow-hidden backdrop-blur">
      <div className="px-4 py-3 border-b border-zinc-800 bg-zinc-900/80">
        ...
        <p className="text-xs text-zinc-400 mt-0.5">{legends.length} documented configs · by clan</p>
      </div>
      <div className="p-2 max-h-[420px] overflow-y-auto">
        {legendGroups.map((group) => (
          <div key={group.clan} className="mb-2 last:mb-0">
            <div className="px-3 py-1 text-[10px] font-mono uppercase tracking-widest text-orange-400/70">
              {group.clan}
            </div>
            {group.ids.map((id) => {
              ...
              return (
                <div key={l.id} className="relative group/legend">
                  <button
                    onClick={() => onLoad(l.id)}
                    className={cn(
                      "w-full text-left px-3 py-2 rounded flex items-center gap-2.5 mb-0.5 transition-all",
