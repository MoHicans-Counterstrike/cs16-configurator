  ...
  return (
    <div className="grid lg:grid-cols-[280px_1fr] gap-4">
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-3">
        <p className="text-xs font-mono text-zinc-300 mb-2">POV Preview</p>
        <div className="relative w-full overflow-hidden rounded-lg border border-zinc-700 shadow-inner shadow-black/60">
          <div className="relative" style={{ paddingBottom: "75%" }}>
            ...
            <svg className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 overflow-visible" width="100" height="100" viewBox="-50 -50 100 100">
        ...
        <div className="flex gap-1 mt-2 flex-wrap">
          {MAP_SCENES.map((s) => (
            <button key={s.id} onClick={() => setScene(s.id)} className={cn("px-2 py-0.5 rounded text-[9px] font-mono border transition-all", scene === s.id ? "border-orange-500 text-orange-300 bg-orange-500/10" : "border-zinc-700 text-zinc-400")}>{s.name}</button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-3">
          ...
          <div className="space-y-1">
            {[
              ...
              <div key={cmd} className="flex items-center justify-between px-2 py-1 rounded bg-zinc-950 border border-zinc-800">
                <code className="text-[10px] font-mono text-orange-300">{cmd}</code>
                <button onClick={() => navigator.clipboard.writeText(cmd)} className="text-[9px] font-mono text-zinc-500 hover:text-orange-300">copy</button>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Gallery Grid */}
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-3">
          <p className="text-xs font-mono text-zinc-300 mb-2">Presets</p>
            ...
            {CROSSHAIR_PRESETS.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setSelected(c.id)} className={cn("aspect-square rounded border flex flex-col items-center justify-center transition-all p-1", selected === c.id ? "border-orange-500 bg-orange-500/10" : "border-zinc-700 hover:border-zinc-500")}>
                  <svg width="24" height="24" viewBox="-12 -12 24 24" className="mb-0.5">
                    <g stroke={`rgb(${cr},${cg},${cb})`} strokeWidth={c.translucent === 2 ? 1 : 2} opacity={c.translucent === 0 ? 1 : c.translucent === 1 ? 0.65 : 0.4}>
                      ...
                      {c.translucent === 2 && <circle cx={0} cy={0} r={1.5} fill={`rgb(${cr},${cg},${cb})`} />}
                    </g>
                  </svg>
                  <span className="text-[7px] font-mono text-zinc-400 truncate w-full text-center">{c.name}</span>
                </button>
              );
            })}
          </div>
