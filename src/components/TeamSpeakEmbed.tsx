  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        setStatus({ online: false, clients: 0, maxClients: 32, channels: 0, name: "Mohicans TS3" });
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-4">
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-4">
        ...
        <div className="mt-3 flex gap-2">
          <a href="ts3server://ts.mohican.xyz" className="px-4 py-2 rounded bg-gradient-to-r from-orange-500 to-amber-500 text-zinc-950 font-bold text-xs font-mono">
            ▶ CONNECT TO TS3
          </a>
          <button
            onClick={() => navigator.clipboard.writeText("ts.mohican.xyz")}
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-4">
        <p className="text-xs font-mono text-zinc-400">
          Join our TeamSpeak server for voice comms with the community.
          All channels are open — hop in and say hi.
        </p>
      </div>
    </div>
  );
